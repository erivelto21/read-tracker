...
- **THEN** the VM SHALL have a public IPv4 address attached to its primary network interface

#### Scenario: SSH key attached
- **WHEN** the VM is provisioned
- **THEN** the configured SSH key SHALL be attached, allowing SSH access
...
The Terraform configuration SHALL output the VM's private IPv4 address and public IPv4 address after a successful `terraform apply`.
